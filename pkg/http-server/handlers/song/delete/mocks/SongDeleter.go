// Code generated by mockery v2.28.2. DO NOT EDIT.

package mocks

import mock "github.com/stretchr/testify/mock"

// SongDeleter is an autogenerated mock type for the SongDeleter type
type SongDeleter struct {
	mock.Mock
}

// DeleteSong provides a mock function with given fields: id
func (_m *SongDeleter) DeleteSong(id int) error {
	ret := _m.Called(id)

	var r0 error
	if rf, ok := ret.Get(0).(func(int) error); ok {
		r0 = rf(id)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

type mockConstructorTestingTNewSongDeleter interface {
	mock.TestingT
	Cleanup(func())
}

// NewSongDeleter creates a new instance of SongDeleter. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewSongDeleter(t mockConstructorTestingTNewSongDeleter) *SongDeleter {
	mock := &SongDeleter{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
